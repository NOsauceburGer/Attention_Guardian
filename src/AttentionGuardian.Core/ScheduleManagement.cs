...
public enum StartTimeConflictResolution
{
    MoveExistingAfterEdited,
    TruncateExistingAtNewStart
}

public enum StartTimeEditRejection
{
    None,
...
public sealed record ScheduleStartEditResult(
    IReadOnlyList<ScheduledTodo> ScheduledTodos,
    StartTimeEditRejection Rejection,
    Guid? ConflictingTodoId);
...
public static class ScheduleManagement
{
    ...
    public static ScheduleStartEditResult EditStart(
        IEnumerable<ScheduledTodo> schedule,
        Guid todoId,
        DateTimeOffset newStart,
        StartTimeConflictResolution? conflictResolution)
    {
        var ordered = ValidateAndOrder(schedule);
        var moving = ordered.SingleOrDefault(todo => todo.Id == todoId)
            ?? throw new ArgumentException("The selected todo does not exist.", nameof(todoId));
        var others = ordered.Where(todo => todo.Id != todoId).ToList();
        var occupying = others.FirstOrDefault(todo => todo.TimeRange.Contains(newStart));

        if (occupying?.IsMandatory == true)
        {
            return new(
                ordered,
        ...
        if (occupying is not null && conflictResolution is null)
        {
            return new(
                ordered,
                StartTimeEditRejection.ConflictResolutionRequired,
                occupying.Id);
        }

        var prefix = others
            .Where(todo => todo.Id != occupying?.Id && todo.TimeRange.End <= newStart)
            .ToList();
        var remaining = others
            .Where(todo =>
                todo.Id != occupying?.Id
                && !prefix.Any(prefixTodo => prefixTodo.Id == todo.Id))
            .ToList();
        var moved = moving.MoveTo(newStart);

        if (occupying is not null
            && conflictResolution == StartTimeConflictResolution.TruncateExistingAtNewStart)
        {
            var truncated = new ScheduledTodo(
                occupying.Id,
                occupying.Title,
                new TimeRange(occupying.TimeRange.Start, newStart),
                occupying.IsMandatory,
                occupying.CurrentSelectionPriority);
            prefix.Add(truncated);
        }

        var desiredSuffix = new List<ScheduledTodo> { moved };
        if (occupying is not null
            && conflictResolution == StartTimeConflictResolution.MoveExistingAfterEdited)
        {
            desiredSuffix.Add(occupying);
        }

        desiredSuffix.AddRange(remaining);
        var rebuiltSuffix = RebuildFromAnchor(desiredSuffix, newStart);
        var finalSchedule = prefix
            .Concat(rebuiltSuffix)
            .OrderBy(todo => todo.TimeRange.Start)
            .ThenBy(todo => todo.TimeRange.End)
            .ThenBy(todo => todo.Id)
            .ToArray();

        return new(finalSchedule, StartTimeEditRejection.None, occupying?.Id);
    }
