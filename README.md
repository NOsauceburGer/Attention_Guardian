...
Windows 客户端使用 Avalonia。后续 macOS 与 iOS 客户端将使用 SwiftUI；两者不共享 UI
代码，而以相同的产品规则与数据契约保持行为一致。

### Apple 开发状态

Apple 客户端目前处于规格确认阶段，尚无可运行的 macOS/iOS 应用、Swift Package 或
Apple 数据库。下一步先建立 C# 与 Swift Core 共用的 JSON 测试向量，不会直接开始 UI。

- 当前交接：[HANDOFF.md](HANDOFF.md)
- 跨平台业务合同：[docs/CROSS_PLATFORM_RULES.md](docs/CROSS_PLATFORM_RULES.md)
- 两套平台设计语言：[DESIGN.md](DESIGN.md)
- 有序实施门槛：[ROADMAP.md](ROADMAP.md)
